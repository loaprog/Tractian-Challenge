...
def train_model(
    ...
    if len(payload.timestamps) != len(payload.values):
        raise HTTPException(status_code=400, detail="timestamps e values devem ter o mesmo tamanho")
    if any(t < 0 for t in payload.timestamps):
        raise HTTPException(status_code=400, detail="timestamps devem ser Unix timestamp válido (>= 0)")
    if payload.timestamps != sorted(payload.timestamps):
        raise HTTPException(status_code=400, detail="timestamps devem estar em ordem crescente")
    if len(set(payload.timestamps)) != len(payload.timestamps):
        raise HTTPException(status_code=400, detail="timestamps não devem conter valores duplicados")
    if len(payload.values) < 2:
        raise HTTPException(status_code=400, detail="Dados insuficientes para treino")
    MAX_POINTS = 100_000
    if len(payload.values) > MAX_POINTS:
        raise HTTPException(status_code=413, detail=f"Série excede limite de {MAX_POINTS} pontos")


    series = TimeSeries(
        data=[DataPoint(timestamp=t, value=v) for t, v in zip(payload.timestamps, payload.values)]
    )
    ...
    if std == 0:
        raise HTTPException(status_code=400, detail="Não é válido treinar modelo com desvio padrão zero")
    ...
    with db.begin():
        ts_db = db.query(TimeSeriesDB).filter_by(series_id=series_id).first()
        if not ts_db:
        ...
        db.query(Model).filter_by(time_series_id=ts_db.id, is_active=True).update({"is_active": False})
    ...
    model.training_latency_ms = latency_ms
    db.commit()  
